...
RC BTreeIndex::recursiveInsert(int curLevel, PageId path[], int key, const RecordId& rid, const PageId& pid) {
    ...
    if(curLevel == treeHeight) {
        ...
        char check [PageFile::PAGE_SIZE];
        pf.read(rootPid, check);
        char rootInfo[PageFile::PAGE_SIZE];
        memcpy(rootInfo, &rootPid, sizeof(PageId));
        memcpy(rootInfo + sizeof(PageId), &treeHeight, sizeof(int));
        pf.write(0, rootInfo);
        char rootInfo2[PageFile::PAGE_SIZE];
        pf.read(0, rootInfo2);
        int treeHeight2;
        memcpy(&treeHeight2, rootInfo2 + sizeof(PageId), sizeof(int));
        return 0;
    }
    if(curLevel == 0) {
        BTLeafNode leaf;
        ...
        if(leaf.getKeyCount() < leaf.MAX_ENTRY_NUM) {
            ...
            if ((rc = leaf.write(path[treeHeight - curLevel - 1], pf)) < 0) {
                return rc;
            }
            return 0;
        }
