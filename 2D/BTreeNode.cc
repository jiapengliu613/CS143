...
RC BTLeafNode::insert(int key, const RecordId& rid) {
        ...
        if(count == 0){
                count++;
                memcpy(buffer, &count, sizeof(int));
                memcpy(buffer + sizeof(int), &rid, sizeof(RecordId));
                memcpy(buffer + sizeof(int) + sizeof(RecordId), &key, sizeof(int));
                return 0;
        }
...
RC BTNonLeafNode::insert(int key, PageId pid) {
    ...
    memmove(buffer + offset + SIZE_OF_KEY_PAGEID, buffer + offset, SIZE_OF_KEY_PAGEID * (keyCount - i - 1));
    memcpy(buffer + offset, &key, sizeof(int));
    memcpy(buffer + offset + sizeof(int), &pid, sizeof(PageId));
    return 0;
}
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey) {
    ...
    memcpy(&keyCountOfSibling, sibling.buffer, sizeof(int));
...
RC BTNonLeafNode::locateChildPtr(int searchKey, PageId& pid) {
    ...
    memcpy(&pid, buffer + offset - sizeof(PageId), sizeof(PageId));
    return 0;
}
...
RC BTNonLeafNode::searchKeyPos (int key, int &i, int &offset, int &keyCount) {
    ...
    for (; i < keyCount; i++, offset+=SIZE_OF_KEY_PAGEID) {
        int curKey;
        memcpy(&curKey, buffer + offset, sizeof(int));
        memcpy(&p, buffer + offset - sizeof(PageId), sizeof(PageId));
