...
$exp = "select id, concat(title, '(', year, ')') as Name from Movie;";
$MovieRes = $db->query($exp);
if (!$MovieRes) {
	$result = "Sth is Wrong with the Movie table";
	die();
}
$exp = "select id, concat(first, ' ', last, '(', dob, ')') as Name from Director;";
$DirectorRes = $db->query($exp);
if (!$DirectorRes) {
	$result = "Sth is wrong with the Director table";
	die();
}
?>
<?php 
if (isset($_GET["submit"])) {
	$did = $_GET["did"];
	$mid = $_GET["mid"];
	$exp = "select * from MovieDirector where mid = '$mid' and did = '$did';";
	$rs = $db->query($exp);
	if ($rs->num_rows > 0) {
		$result = "It's already in the table!";
	} else {
		$exp = "insert into MovieDirector values ('$mid', '$did');";
		$rs = $db->query($exp);
		if (!$rs) {
